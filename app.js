(function() {
    ...
    const RESULT_TYPES = [
        { minScore: 6, maxScore: 8, label: '时差型・国际闲人', subtitle: '作息紊乱，白天特困', desc: '你的生物钟横跨五个时区，白天靠咖啡续命，晚上靠手机续魂。', radar: [2, 3, 2, 3, 2], product: { name: '酸枣仁睡前膏', desc: '安神助眠，睡前服用一勺，一觉睡到自然醒', url: 'https://v.douyin.com/DMz6pYV0Khg/' } },
        { minScore: 9, maxScore: 10, label: '积食型・外卖品鉴官', subtitle: '胃胀不消化，积食严重', desc: '外卖评分4.9，肠胃评分负分。每一口都是对消化系统的极限挑战。', radar: [3, 1, 3, 4, 2], product: { name: '小调皮山楂膏', desc: '消食化积，饭后来一勺，肠胃轻松无负担', url: 'https://v.douyin.com/sOAZfHSNPVE/' } },
        { minScore: 11, maxScore: 12, label: '水肿型・人间注水肉', subtitle: '湿气重，浮肿易累', desc: '起床像泡发的银耳，按一下小腿回弹要三秒——湿气，退！退！退！', radar: [3, 3, 2, 3, 1], product: { name: '陈皮茯苓膏', desc: '健脾祛湿，每天两杯，告别水肿一身轻', url: 'https://v.douyin.com/tjeEtIYNTag/' } },
        { minScore: 13, maxScore: 14, label: '摆烂型・行走灭火器', subtitle: '气虚乏力，精力不足', desc: '说话像0.5倍速，能坐着不站着，能躺着不坐着——你是节能环保代言人。', radar: [3, 3, 1, 3, 3], product: { name: '黄精元芪膏', desc: '补气养血，每天一勺，元气满满一整天', url: 'https://v.douyin.com/P_KLcvF3ebY/' } },
        { minScore: 15, maxScore: 16, label: '易燃型・行走小爆竹', subtitle: '肝火旺，情绪易爆', desc: '三句话点燃，两句话后悔，内心小剧场演完八十集，只剩疲惫。', radar: [3, 4, 3, 1, 3], product: { name: '小心甘养生茶花茶', desc: '疏肝解郁，服用后心平气和，看什么都顺眼', url: 'https://v.douyin.com/miCQ0_4itsY/' } },
        { minScore: 17, maxScore: 18, label: '精致型・脆皮塑料花', subtitle: '外强中干，气血不足', desc: '妆容精致，体检单惊心。换季必感冒，姨妈常迟到。', radar: [2, 3, 2, 2, 3], product: { name: '玉容玫白膏', desc: '补气养血，调经美容，内外兼修做精致女孩', url: 'https://v.douyin.com/qCX8lW-9WC8/' } },
        { minScore: 19, maxScore: 20, label: '朋克型・养生刺客', subtitle: '无效养生，自我安慰', desc: '啤酒泡枸杞，熬夜敷面膜，你的养生主打一个\'来都来了\'。', radar: [2, 2, 3, 2, 2], product: { name: '纤荷茶', desc: '综合调理，阴阳双补，全面拯救你的亚健康', url: 'https://v.douyin.com/U9areEs3vMc/' } },
        { minScore: 21, maxScore: 22, label: '修仙型・秃头预警选手', subtitle: '过度内耗，身心透支', desc: '灵修课上了三节，头发掉了三把。精神升天，肉身坠地。', radar: [1, 2, 1, 1, 3], product: { name: '益椹黄精膏', desc: '安神养心，补肾益精，身心同调找回状态', url: 'https://v.douyin.com/_ngbL84K-v8/' } },
        { minScore: 23, maxScore: 23, label: '开摆型・人间懒羊羊', subtitle: '极度懒动，代谢偏低', desc: '能语音绝不打字，能外卖绝不出门。你的养生哲学：少消耗就是赢。', radar: [3, 3, 2, 3, 3], product: { name: '清润养生茶', desc: '开袋即食，无需煎煮，随时随地养生偷懒两不误', url: 'https://v.douyin.com/UKm5KbLJ6BA/' } },
        { minScore: 24, maxScore: 24, label: '强迫型・健康卷王', subtitle: '养生焦虑，过度紧绷', desc: '每天打卡八杯水、十种保健品，越养越累，越累越养。', radar: [2, 3, 2, 1, 3], product: { name: '本草茶', desc: '放松神经，疏解压力，停止焦虑享受生活', url: 'https://v.douyin.com/Wfr5h-6Szis/' } }
    ];
    ...
    function renderResult() {
        ...
        // 只显示人格类型，不显示完整人格标签
        const personalityType = result.label.split('・')[0];
        document.getElementById('result-label').textContent = personalityType;
        document.getElementById('result-subtitle').textContent = result.isHidden ? '🎉 恭喜你发现了彩蛋！' : result.subtitle;
        document.getElementById('result-desc').textContent = result.desc;

        // 保持门诊挂号咨询信息不变，不再显示产品信息
        // 移除产品链接按钮，因为我们已经在卡片内部添加了预约按钮
    ...
    function generatePoster() {
        ...
        // 只显示人格类型，不显示完整人格标签
        const personalityType = state.result.label.split('・')[0];
        ...
        posterContent.innerHTML = `
            ...
            <div class="poster-label">${personalityType}</div>
    ...
    function bookConsult() {
        ...
        window.location.href = 'http://hzfhct.cn/gnU5m';
    }
